...
function AtmosphereDefinitions({ id }: { id: AtmosphereVariant }) {
  return <defs>
...
function HeroAtmosphere({ variant }: { variant: 'hero' | 'contact' }) {
  ...
  if (variant === 'contact') {
    return <svg className="atmosphere-art" viewBox="0 0 1440 1080" preserveAspectRatio="xMidYMid meet" focusable="false">
      <AtmosphereDefinitions id={variant} />
      <path d={HERO_PATH} fill={gradient} filter={blur} transform="translate(-814 -112)" opacity=".9" />
    </svg>;
  }

  return <svg className="atmosphere-art" viewBox="0 0 1440 1359" preserveAspectRatio="xMidYMid meet" focusable="false">
    <AtmosphereDefinitions id={variant} />
    <path d={HERO_PATH} fill={gradient} filter={blur} transform="translate(64 43.5)" />
  </svg>;
}
...
function DreamAtmosphere() {
  return <svg className="atmosphere-art" viewBox="0 0 1007 1014" preserveAspectRatio="xMidYMid meet" focusable="false">
    <AtmosphereDefinitions id="dream" />
    <path d={DREAM_PATH} fill="url(#dream-atmosphere-gradient)" filter="url(#dream-atmosphere-blur)" transform="translate(-109 165.5)" />
  </svg>;
}
